...
public class ChampionManagerBase : SingletonManager<ChampionManagerBase> {
    ...
    private List<Champion> championsBoughtLastCombatRound;//在战斗阶段买的所有英雄都会放到这里面,战斗阶段开始清零,deploy阶段开始的之后会挨个检测是否能合成
    ...
    public void RegisterChampion(Champion champion) {
        ...
        if(GameManager.Instance.PlayState.ActiveState.name == OnPlayState.COMBAT) {
            Instance.championsBoughtLastCombatRound.Add(champion);//如果在战斗中买的,要加进来
        }
        CheckChampionUpgrade(champion);
    }
    public void UnregisterChampion(Champion champion) {
        InitDict();
        if(!championsDict.ContainsKey(champion.ChampionName)) {
    ...
    public static bool CanThisChampionUpgrade(Champion champion) {
    ...
    private static void CheckChampionUpgrade(Champion champion) {
        ...
        from item in championsDict[champion.ChampionName]
        where item.Level == 0 && (item.LastQuadThisChampionStand is PreparationQuad || Instance.championsBoughtLastCombatRound.Contains(item)) 
        ...
        GameManager.Instance.PlayState.ActiveState.name == OnPlayState.COMBAT?
        from item in championsDict[champion.ChampionName]
        where item.Level == 1 && (item.LastQuadThisChampionStand is PreparationQuad || Instance.championsBoughtLastCombatRound.Contains(item))
        select item : null;
    ...
    private void OnEnterCombatState(GameEventTypeVoid ev) {
        if(championsBoughtLastCombatRound.Count > 0) {//战斗开始的时候清空
            championsBoughtLastCombatRound.Clear();
        }
    }
    private void OnEnterDeployState(GameEventTypeVoid ev) {//开始买东西之前把每个champion都检查一遍
